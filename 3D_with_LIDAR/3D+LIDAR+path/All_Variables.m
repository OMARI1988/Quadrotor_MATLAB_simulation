...
%-------- Path ------------
A.var2 = [0:.2:2*pi];
A.X_path = [linspace(0,2,4) 2 2 linspace(2,-1.5,7) linspace(-1.5,-1.5,2)];   % the desired path in X
A.Y_path = [linspace(0,0,4) .5 1 linspace(1.5,1.5,7) linspace(2,2.5,2)];   % the desired path in Y
% A.X_path = sin(A.var2);
% A.Y_path = cos(A.var2)-1;
A.Z_path = linspace(0,1.6,length(A.X_path));   % the desired path in Z

A.path_counter = 1;     % manage the path points
A.flagggg=0;
...
% Desired variables
A.Z_des = 0.5;            % desired value of Z
